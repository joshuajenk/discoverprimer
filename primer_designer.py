...
class PrimerDesigner:
    ...
    def run_cli(self):
        ...
        # custom
        if choice.lower() in ('custom', 'c', 'custom sequence', 'cus'):
            custom_seq = input(
                "Enter custom sequence (100–1000 bp; DNA or AA): "
            ).strip()
            if not custom_seq:
            ...
            # detect AA v DNA
            if self.is_amino_acid_sequence(custom_seq):
                print("Amino-acid sequence detected—converting to DNA.")
                seq = self.amino_acid_to_dna(custom_seq)
                print(f"Converted AA → DNA ({len(seq)} bp).")
            else:
    ...
    def analyze_primer_quality(self, seq, organism='Homo sapiens', database='refseq_rna'):
        quality = {
            'sequence': seq,
            'length': len(seq),
            'tm': round(calc_tm_nn(seq, Default_NA, Default_MG), 1),
            'gc_content':  round((seq.count('G') + seq.count('C')) / len(seq) * 100, 1),
            'hairpins': self.check_hairpin(seq),
            'self_dimers': self.check_dimers(seq),
            'specificity_analysis': self.check_specificity(seq, organism, database),
            'warnings': []
        }
        quality['specificity_issues'] = quality['specificity_analysis'].get('hits', [])

        # warnings on analysis
        # warning on melt temperature - adjust in primer_config.py
