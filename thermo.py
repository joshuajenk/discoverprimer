...
def calc_tm_nn(seq, primer_conc = Default_PRIMER_CONC, na_conc=50, mg_conc=1.5):
    seq = seq.upper()
    H, S = 0.0, 0.0
    ...
    # Tm calc
    denom = S + R * math.log(primer_conc / 4.0)
    if denom == 0:
        return 0.0
    tm_K = (1000 * H) / denom
    tm_C = tm_K - 273.15 + salt_corr
    return round(tm_C, 1)

def deltaG(seq1, seq2=None, na_conc=50, mg_conc=1.5):
    # duplex formation
